/**
 * DTO для создания простого заказа (Simple Order).
 * 
 * Назначение:
 * - Используется для передачи параметров создания заказа с простыми услугами от клиента к backend.
 * - Применяется в контроллерах и сервисах для создания заказа через API.
 * 
 * Содержит данные:
 * - список услуг, данные клиента, параметры оплаты и др.
 */
